...
<div class="card">
        <div class="card-header" style="display:flex;">
            ...
            <!-- Search form -->
                    <form class="form-inline ml-auto" method="GET" action="">
                        <input class="form-control form-control-sm mr-3 w-75" name="search" type="text" placeholder="Search by Dept. or Batch" aria-label="Search" >
                        <button type="submit" class="btn btn-primary"> <i class="fas fa-search" aria-hidden="true"></i></button>
                    </form>
            <?php if (isset($_GET['search'])){?>
                <a href="alumnnireportsearch.php?search=<?php echo $_GET['search'];?>" class="btn btn-success" target="_blank"> Print Selected Alumni</a>   
            <?php }else{?>
                <a href="alumnnireport.php" class="btn btn-success" target="_blank"> Print Alumni List</a> 
            <?php 
            }?>
        ...
        if (isset($_GET['search'])){
                    $srch=$_GET['search'];
                      $qry = "SELECT users.*, departments.name as department_name, `batches`.name as batch_name FROM `users` 
                        LEFT JOIN departments ON users.dept_id=departments.id 
                        LEFT JOIN `batches` ON users.batch_id=`batches`.id
                        WHERE departments.name LIKE '%$srch%' 
                        OR batches.name LIKE '%$srch%'
